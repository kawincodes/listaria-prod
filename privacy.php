...
if (!$text) {
    $text = "Privacy Policy content not available.";
}

// Reuse similar parser logic
function parseLegalDoc($text) {
    ...
    foreach ($lines as $line) {
        ...
        // Section Headers (e.g., "1. Data Collection")
        ...
        // Emoji Headers (e.g., "🔐 Security")
        elseif (preg_match('/^[\x{1F300}-\x{1F6FF}].*/u', $line)) {
            if ($inList) { $contentHtml .= '</ul>'; $inList = false; }
        ...
        // Regular Text
            ...
            if (stripos($line, 'Last Updated:') === 0 || stripos($line, 'Effective Date:') === 0) {
                $contentHtml .= '<div class="legal-meta-badge">' . htmlspecialchars($line) . '</div>';
            }
            elseif (stripos($line, 'Privacy') !== false && strlen($line) < 50) {
