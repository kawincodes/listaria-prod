...
//                        $response[$key]['is_liked'] = $collection->favourites->where(['item_id' => $collection->id, 'user_id' => Auth::user()->id])->count() > 0;
                ...
                // Prepare translated item safely
                $translatedItem = [
                    ...
                    $response[$key]['translated_address'] = $response[$key]['translated_city'].', '.$response[$key]['translated_state'].', '.$response[$key]['translated_country'];
                    ...
                    foreach ($grouped as $customFieldId => $fieldValues) {
                        ...
                        // ➤ Translated (Content-Language preferred)
