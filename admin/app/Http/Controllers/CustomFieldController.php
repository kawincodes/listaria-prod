...
class CustomFieldController extends Controller {
    ...
    public function create(Request $request) {
        ...
        return view("custom-fields.create", compact('categories', 'cat_id','languages', 'selected_categories', 'selected_all_categories'));
    }
    ...
    public function store(Request $request)
    {
        ...
        if (in_array($request->type, ['number', 'textbox'])) {
            ...
            $baseRules['max_length'] = 'nullable|integer|min:0|gte:min_length';
        }
    ...
    public function edit($id) {
       ...
       $selected_categories = $custom_field->custom_field_category->pluck('category_id')->toArray();
        ...
        foreach ($selected_categories as $catId) {
            $categoryId = $catId;
            while ($categoryId) {
                $parent = \App\Models\Category::where('id', $categoryId)->value('parent_category_id');
                if ($parent) {
    ...
    public function update(Request $request, $id) {
        ...
        try {
           ...
           $originalValues = $request->input('values', []);
                $languages = CachingService::getLanguages()->where('code', '!=', 'en')->values();
                ...
                foreach ($languages as $language) {
                    ...
                    $translatedName = $request->input("translations.$langId.name");
                    $translatedValues = $request->input("translations.$langId.value", []);
