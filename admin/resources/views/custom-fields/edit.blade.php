                                ...
                                <div class="row mt-3">
                                   ...
                                   @if($languages->isNotEmpty())
                                            ...
                                            @foreach($languages as $language)
                                                <div class="col-md-12 mb-4 border p-3 rounded shadow-sm field-value-translation-wrapper" id="translation-wrapper-{{ $language->id }}">
                                                    <h6 class="mb-3 text-primary">
                                                        {{ __("Translation for") }}: <strong>{{ $language->name }} ({{ $language->code }})</strong>
                                                    </h6>

                                                    <div class="form-group mb-3">
                                                        <label for="translations_{{$language->id}}_name" class="form-label">
                                                            {{ __("Field Name") }}
                                                        </label>
                                                        <input type="text" class="form-control"
                                                            id="translations_{{$language->id}}_name"
                                                            name="translations[{{$language->id}}][name]"
                                                            placeholder="{{ __('Name in') . ' ' . $language->name }}"
                                                            value="{{ $translations[$language->id]['name'] ?? '' }}" required>
                                                    </div>

                                                    <div class="form-group field-value-translation" id="field-value-translation-{{ $language->id }}" style="display: none;">
                                                        <label class="form-label">{{ __("Field Values") }}</label>
                                                        <div class="row translated-values-container" data-language-id="{{ $language->id }}">
                                                        </div>
                                                        <small class="text-danger error-msg-{{ $language->id }}"></small>
                                                        <div class="input_hint mt-2">
                                                            {{ __("This will be applied only for") }}:
                                                            <text class="highlighted_text">{{ __("Checkboxes") }}, {{ __("Radio") }}, {{ __("Dropdown") }}</text>.
                                                        </div>
                                                    </div>
                                                </div>
                                            @endforeach
